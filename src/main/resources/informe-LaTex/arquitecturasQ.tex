...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Características generales (susi)} 
\ojo{ Dar forma de prosa!
\begin{itemize}
\item Tiene \textbf{8 registros} de uso general de 16 bits, denominados \textbf{R0..R7}.

\item La memoria tiene \textbf{direcciones de 16 bits}.

\item Tiene un contador de programas de 16 bits.
...
\end{itemize}
}{La arquitectura Q tiene 8 registros de uso general de 16 bits, denominados R0...R7, los registros especificos incluyen un PC bla bla, un SP bla bla, un registro de flags bla bla... Todas las instrucciones alteran los flags...}

\ojo{\textbf{Nota1:} El caracter \% denota el cociente de la división entera\\
...
\item \textbf{Directo}
\ojo{Este modo de direccionamiento representa}{Con este modo de direccionamiento se denota un operando alojado en} una dirección de memoria o de puertos\ojo{que será utilizado como modo origen pero nunca como modo destino ya que en el no pueden guardarse datos.}{no va!}La codificación de este modo se indica en la tabla \ref{tablamodos}.
...
\ojo{Este modo de direccionamiento representa una dirección de memoria o de puertos que será utilizado como modo origen pero nunca como modo destino ya que en el no pueden guardarse datos.}{Con este modo de direccionamiento se denota un operando alojado en una celda de memoria cuya dirección está almacenada en otra celda de memoria.} La codificación de este modo se indica en la tabla \ref{tablamodos}.
...
\item \textbf{Registro}\ojo{Donde "rrr" es una cadena binaria que puede tomar los valores desde 000 hasta 111 para identificar según el numero decimal que representen los diferentes ocho registros R0..R7.}{Con este modo de direccionamiento se denota un operando alojado en un registro de uso general (R0 a R7)} La codificación de este modo se indica en la tabla \ref{tablamodos}.

Ejemplos:
\textbf{R0} \ojo{representa el modo de direccionamiento registro cuyo valor se encuentra en el registro \textbf{R0}.}{denota un operando almacenado en el registro R0} 
\textbf{R7} \ojo{representa el modo de direccionamiento registro cuyo valor se encuentra en el registro \textbf{R7}.}{denota un operando almacenado en el registro R7}

\item \textbf{RegistroIndirecto} \ojo{Donde "rrr" es una cadena binaria que puede tomar los valores desde 000 hasta 111 para identificar según el numero decimal que representen los diferentes ocho registros R0..R7.}{De manera similar al modo indirecto, con este modo de direccionamiento se denota un operando alojado en una celda de memoria cuya dirección está almacenada en el registro indicado} La codificación de este modo se indica en la tabla \ref{tablamodos}.


Ejemplos:
\textbf{[R0]} \ojo{representa el modo de direccionamiento registro cuyo valor se encuentra en la direccion de memoria que esta guardada como dato el registro \textbf{R0}.}{denota un operando almacenado en una celda de memoria cuya dirección está en el registro R0}
\textbf{[R7]} \ojo{representa el modo de direccionamiento registro cuyo valor se encuentra en la direccion de memoria que esta guardada como dato el registro \textbf{R7}.}{denota un operando almacenado en una celda de memoria cuya dirección está en el registro R7}
...
\ojo{El formato de instruccion}{A continuación se muestra la codificación (formato) de las instrucciones} de dos operandos:
...
\ojo{Donde el operando destino no puede ser un modo de direccionamiento Inmediato, ya que en la dirección de memoria o registro que describa dicho operando es donde se guardará el resultado de la operación.}{(Explicar un poco mas) Las instrucciones de dos operandos descriptas a continuación son instrucciones aritméticas o lógicas donde se asume que el resultado de la operación se almacena en uno de los dos operandos de entrada, y por lo tanto se lo denomina \textbf{operando destino}.}

\begin{enumerate}
\item \textbf{MUL destino, origen}
Código de operación: 0000
Esta instrucción describe la multiplicación entre los datos de los dos operandos. Esta operación es la única que cuyo resultado puede ser 32 bits, que son \ojo{dos celdas de memoria en hexadecimal}{No entiendo!}, por lo que los primeros 16 bits, es decir, la primer mitad, es guardada en el registro R7 y la segunda en el operando destino.
